...
def checkin_attendance_creation(data):
	...
	frappe.log_error(message=data, title="Checkin Data")
	# checkin_from = ''
	# if data.office == 1:
	...
	if not data.get("employee"):
		...
		eid = frappe.get_doc("Employee",data.get("employee"))

	if eid:
		emp_check_in.employee = eid.name
		emp_check_in.employee_name = eid.employee_name
		emp_check_in.company = eid.company
		...
		if data.get("longitude"):
			emp_check_in.longitude = data.get("longitude")
		if data.get("city"):
			emp_check_in.city = data.get("city")
		if data.get("state"):
			emp_check_in.state = data.get("state")
		if data.get("latitude"):
			emp_check_in.latitude = data.get("latitude")
		...
		try:
			emp_check_in.save()
			return {"status":1,"doc":emp_check_in.name}
		except Exception:
			error_log = frappe.log_error(message=frappe.get_traceback(), title="Checkin Error")
			frappe.msgprint(_("Error Occured"))
			return {"status":0,"doc":error_log.name}
		...
		return {"status":2,"doc":"Employee Does not Exist"}
