...
def update_shift_details(attendance_dict):
    shift_time_in_hours = frappe.utils.get_time('0:0:0')
    ...
    if shift_start_time and shift_end_time:
        shift_time_in_hours = frappe.utils.time_diff(shift_end_time,shift_start_time)
        attendance_dict['expected_working_hours'] = shift_time_in_hours
    else:
        attendance_dict['expected_working_hours'] =frappe.utils.get_time('0:0:0')
    ...
    if checkin_time and checkout_time:
        attendance_dict['working_hours'] = frappe.utils.time_diff(checkout_time,checkin_time)
    else:
        attendance_dict['working_hours'] =frappe.utils.get_time('0:0:0').hour

    # get working hour deviation
    if attendance_dict['working_hours'] and attendance_dict['expected_working_hours']:
    	attendance_dict['overtime'] = frappe.utils.time_diff(attendance_dict['working_hours'] ,attendance_dict['expected_working_hours'])
